...
router.post('/', requireRole('Admin'), authorize('users', 'create'), async (req, res) => {
  try {
    const tenantId = resolveTenantId(req);
    const { name, email, password, role } = req.body || {};
    const persistedRole = persistRole(role);
    const normalizedTargetRole = normalizeRole(persistedRole);
    const creatorRole = normalizeRole(req.user && req.user.role);
    ...
    if (normalizedTargetRole !== 'IT_SUPPORT') {
      return res.status(400).json({
        success: false,
        message: 'POST /users currently supports creating only IT Support users'
      });
    }
    ...
    if (!['SUPER_ADMIN', 'ADMIN'].includes(creatorRole)) {
      return res.status(403).json({
        success: false,
        message: 'Only Super Admin or Admin can create IT Support users'
      });
    }
    ...
    const existing = await queryAsync(
      'SELECT _id FROM users WHERE email = ? AND tenant_id = ? LIMIT 1',
      [email, tenantId]
    );
    ...
    const title = 'IT Support';

    const fields = ['tenant_id', 'public_id', 'name', 'email', 'password', 'role', 'title', 'isActive', 'isGuest'];
    const placeholders = ['?', '?', '?', '?', '?', '?', '?', '?', '?'];
    const params = [tenantId, publicId, name, email, hashedPassword, persistedRole, title, 1, 0];

    if (await tableHasColumn('users', 'created_by')) {
      fields.push('created_by');
      placeholders.push('?');
      params.push(req.user._id);
    }

    if (await tableHasColumn('users', 'createdAt')) {
      fields.push('createdAt');
      placeholders.push('NOW()');
    }

    if (await tableHasColumn('users', 'updatedAt')) {
      fields.push('updatedAt');
      placeholders.push('NOW()');
    }

    if (await tableHasColumn('users', 'created_at')) {
      fields.push('created_at');
      placeholders.push('NOW()');
    }

    if (await tableHasColumn('users', 'updated_at')) {
    ...
    const rows = await queryAsync(
      `
        SELECT _id, public_id, tenant_id, name, email, role, title, isActive, isGuest
        FROM users
        WHERE _id = ?
        LIMIT 1
      `,
    ...
    logger.error('Create IT Support user error:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to create IT Support user',
      error: error.message
    });
  }
});
